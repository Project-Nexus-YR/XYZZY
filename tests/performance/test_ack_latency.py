...
async def test_file_backed_selection_acknowledgement_p95_below_250_ms(tmp_path: Path) -> None:
    ...
    # This is an acceptance gate, not just telemetry: the repository contract
    # requires local p95 below 250 ms alongside the durability assertions.
    print(f"selection acknowledgement p95: {p95_ms:.3f} ms")
    assert p95_ms < 250, f"selection acknowledgement p95 exceeded the 250 ms bar: {p95_ms:.3f}"
