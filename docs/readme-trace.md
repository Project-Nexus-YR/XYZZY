...
| "1063 tests (1062 passing, 1 skipped without OPENAI_API_KEY)" (Current Status) | `.venv/Scripts/python.exe -m pytest -q -p no:cacheprovider`, rerun 2026-09-04 after the audit fixes | exit code 0; 1063 collected (counted from the `.`/`s` result characters across all report lines, summed with the `--collect-only -q` per-file counts, both totalling 1063); 1 `SKIPPED [1] tests/integration/test_live_provider.py:31: live provider test needs OPENAI_API_KEY` |
