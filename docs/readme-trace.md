...
| CI runs ruff/mypy/pytest on 3.11 and 3.14, pinned actions | `cat .github/workflows/ci.yml` | job runs `ruff check`, `ruff format --check`, `mypy src`, `pytest` under a `python-version: ["3.11", "3.14"]` matrix, `uses:` lines pinned to commit SHAs |
...
| 66 commits since v0.4.0 (CHANGELOG Unreleased), re-measured 2026-09-05 | `git log v0.4.0..HEAD --oneline | wc -l` | `66` (measured as the commit that refreshed this row landed) |
